 ...
 *
 * (generated by 1st gen of the pg2arrow; will be deprecated in the future version)
 * ----------------------------------------------------------------
 */
...
/* ----------------------------------------------------------------
 *
 * assignArrowStatsBinary
 *
 * It assigns per-record-batch min/max statistics generated by 2nd generation of
 * the pg2arrow. It embeds key-value pairs for each target fields.
 * In the future version, only this mechanism will be maintained.
 * ----------------------------------------------------------------
 */
static void
	...
	*__max_token++ = '\0';
	...
	switch (field->type.node.tag)
	{
		...
		case ArrowNodeTag__Timestamp: {
			...
			{
				stats->isnull = false;
				stats->min.datum = Int64GetDatum(__min);
				stats->max.datum = Int64GetDatum(__max);
			}
			break;
		}
		case ArrowNodeTag__FloatingPoint: {
			double	__min = strtod(__min_token, &end1);
			double	__max = strtod(__max_token, &end2);

			if (*end1 != '\0' || *end2 != '\0')
				stats->isnull = true;
			else if (field->type.FloatingPoint.precision == ArrowPrecision__Half)
			{
				stats->isnull    = false;
				stats->min.datum = fp64_to_fp16(__min);
				stats->max.datum = fp64_to_fp16(__max);
			}
			else if (field->type.FloatingPoint.precision == ArrowPrecision__Single)
			{
				stats->isnull    = false;
				stats->min.datum = Float4GetDatum((float)__min);
				stats->max.datum = Float4GetDatum((float)__max);
			}
			else if (field->type.FloatingPoint.precision == ArrowPrecision__Double)
			{
				stats->isnull    = false;
				stats->min.datum = Float8GetDatum(__min);
				stats->max.datum = Float8GetDatum(__max);
			}
			else
			{
				stats->isnull = true;
		...
		case ArrowNodeTag__Decimal:
			PG_TRY();
			{
				Datum	__min = DirectFunctionCall3(numeric_in,
													CStringGetDatum(__min_token),
													ObjectIdGetDatum(InvalidOid),
													Int32GetDatum(-1));
				Datum	__max = DirectFunctionCall3(numeric_in,
...
static inline void
assignArrowStatsBinary(RecordBatchState *rb_state,
					   ...
					   ArrowKeyValue *custom_metadata)
{
	for (int i=0; i < num_custom_metadata; i++)
	{
		...
		if (kv->key && kv->value && strncmp(kv->key, "min_max_stats.", 14) == 0)
		{
			const char *field_name = (kv->key + 14);
							...
							Bitmapset **p_stat_attrs)
{
	...
	for (int i=0; i < af_info->footer._num_recordBatches; i++)
	{
		...
		ArrowMessage	   *message = &af_info->recordBatches[i];
		ArrowRecordBatch   *rbatch = &message->body.recordBatch;
		RecordBatchState   *rb_state;
		...
		if (arrow_bstats)
			...
			assignArrowStatsBinary(rb_state, p_stat_attrs,		/* stats v2 */
								   &af_info->footer.schema,
								   message->_num_custom_metadata,
